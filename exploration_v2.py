...
class Exploration:
    ...
    def top_grams_in_date_range(self, parquet_file, start_date, end_date, top_n):
        """
        Stream a Parquet file to get the top N words within a specific date range.

        Args:
            parquet_file (str): Path to the Parquet file.
            start_date (str): Start date of the range (inclusive) in 'YYYY-MM-DD' format.
            end_date (str): End date of the range (inclusive) in 'YYYY-MM-DD' format.
            top_n (int): Number of top words to retrieve.

        Returns:
            pd.DataFrame: DataFrame containing the top N words and their counts within the date range.
        """
        # Hardcoded column names
        date_column = 'date'
        word_column = 'ngram'
        count_column = 'total_frequency'

        try:
            # Read the Parquet file in chunks using pyarrow
            ...
            grouped = combined_data.groupby(word_column)[count_column].sum().reset_index()

            # Sort by count and get top N words
            top_words = grouped.sort_values(by=count_column, ascending=False).head(top_n)
    ...
    def keyword_search_in_date_range(self, parquet_file, keyword, start_date, end_date):
        """
        ...
        try:
            ...
            for batch in parquet_file_reader.iter_batches():
                ...
                keyword_filtered = filtered_chunk[filtered_chunk[word_column].str.contains(keyword, case=False, na=False)]

                # Append filtered data
                filtered_data.append(keyword_filtered)
            ...
            if filtered_data:
                ...
                result = combined_data.groupby(date_column)[count_column].sum().reset_index()
                print(self._get_ram_status())
                return result
            else:
                # Print error message if no matches are found
                print("No keywords were found for the specified date range.")
                return None
    ...
    def emoji_search_in_date_range(self, parquet_file, keyword, start_date, end_date):
        """
        ...
        Returns:
            pd.DataFrame: DataFrame with 'date', 'ngram', and 'total_frequency' for n-grams containing the keyword,
        ...
        try:
            ...
            for batch in parquet_file_reader.iter_batches():
                ...
                date_mask = (chunk[date_column] >= start_date) & (chunk[date_column] <= end_date)

                # Ensure 'ngram' is string type to use .str accessor
                if not pd.api.types.is_string_dtype(chunk[word_column]):
                    chunk[word_column] = chunk[word_column].astype(str)

                # Filter by keyword (whole word, case-insensitive)
                keyword_mask = chunk[word_column].str.contains(keyword, case=False, na=False, regex=False)

                # Combine masks
                combined_mask = date_mask & keyword_mask

                # Filter data based on combined mask
                filtered_chunk = chunk.loc[combined_mask]
            ...
            if filtered_data:
                ...
                result_df = combined_data[[date_column, word_column, count_column]].sort_values(by=date_column)
                print(self._get_ram_status())
                return result_df
            else:
    ...
    def keyword_search_with_ratios_in_date_range(self, parquet_file, keyword, start_date, end_date):
        """
        ...
        try:
            ...
            for batch in parquet_file_reader.iter_batches():
                ...
                required_columns = [date_column, word_column] + frequency_columns
                ...
                date_mask = (chunk[date_column] >= start_date) & (chunk[date_column] <= end_date)
                keyword_mask = chunk[word_column].str.contains(f'\b{keyword}\b', case=False, na=False, regex=False)
                combined_mask = date_mask & keyword_mask
            ...
            if filtered_data:
                ...
                summed_frequencies = combined_data[frequency_columns].sum()
    ...
    def emoji_search_with_ratios_in_date_range(self, parquet_file, keyword, start_date, end_date):
        """
        ...
        try:
            ...
            for batch in parquet_file_reader.iter_batches():
                ...
                # Filter by date range and keyword (whole word, case-insensitive)
                date_mask = (chunk[date_column] >= start_date) & (chunk[date_column] <= end_date)
                keyword_mask = chunk[word_column].str.contains(f'\b{keyword}\b', case=False, na=False, regex=True)
                combined_mask = date_mask & keyword_mask
    ...
    def plot_keyword_frequencies_comparison(self, parquet_file: str, keywords_list: List[str], start_date: str, end_date: str) -> None:
        """
        ...
        try:
            ...
            if not pd.api.types.is_string_dtype(data['ngram']):
                data['ngram'] = data['ngram'].astype(str)

            # Filter data by the overall date range first for efficiency
            date_mask_overall = (data['date'] >= start_date) & (data['date'] <= end_date)
            relevant_data = data[date_mask_overall]
    ...
    def plot_top_n_grams_trend(self, parquet_file: str, start_date: str, end_date: str, top_n: int = 10) -> None:
        """
        ...
        try:
            ...
            date_mask_overall = (data['date'] >= start_date) & (data['date'] <= end_date)
            period_data = data[date_mask_overall]
...
# --- Hashtag Specific Methods ---
    def top_hashtags_in_date_range(self, parquet_file_path: str, start_date: str, end_date: str, top_n: int = 20) -> Optional[pd.DataFrame]:
        """
        ...
        Parameters:
            parquet_file_path (str): Path to the Parquet file containing the data.
            start_date (str): Start date for filtering (inclusive).
            end_date (str): End date for filtering (inclusive).
            top_n (int): Number of top hashtags to return (default is 20).

        Returns:
            Optional[pd.DataFrame]: DataFrame containing top hashtags and their frequencies, or None if an error occurs.
        """
        try:
            ...
            # Group by 'hashtag' and sum frequencies
    ...
    def search_hashtag_in_date_range(self, parquet_file_path: str, hashtag_to_search: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        """
        ...
        Parameters:
            parquet_file_path (str): Path to the Parquet file containing the data.
            hashtag_to_search (str): The hashtag to search for.
            start_date (str): Start date for filtering (inclusive).
            end_date (str): End date for filtering (inclusive).

        Returns:
            Optional[pd.DataFrame]: DataFrame containing the filtered data, or None if an error occurs.
        """
        try:
            # Read data from the Parquet file
            data = pd.read_parquet(parquet_file_path)
            ...
            # Ensure 'hashtag' column exists and is string type
            if 'hashtag' not in data.columns or not pd.api.types.is_string_dtype(data['hashtag']):
                data['hashtag'] = data['hashtag'].astype(str)

            # Ensure 'total_frequency' column exists
            if 'total_frequency' not in data.columns:
                print("Error: 'total_frequency' column not found.")
                return None

            # Apply date and hashtag filters
            date_mask = (data['date'] >= start_date) & (data['date'] <= end_date)
            keyword_mask = data['hashtag'].str.contains(f'\\b{hashtag_to_search}\\b', case=False, na=False, regex=True)
            combined_mask = date_mask & keyword_mask
    ...
    def search_hashtag_total_frequency_in_range(self, parquet_file_path: str, hashtag_keyword: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        """
        ...
        Parameters:
            parquet_file_path (str): Path to the Parquet file containing the data.
            hashtag_keyword (str): The hashtag to search for.
            start_date (str): Start date for filtering (inclusive).
            end_date (str): End date for filtering (inclusive).

        Returns:
            Optional[pd.DataFrame]: DataFrame containing the total frequency sum, or None if an error occurs.
        """
        try:
            # Read data from the Parquet file
            data = pd.read_parquet(parquet_file_path)
            ...
            # Apply date and hashtag filters
            date_mask = (data['date'] >= start_date) & (data['date'] <= end_date)
            keyword_mask = data['hashtag'].str.contains(f'\\b{hashtag_keyword}\\b', case=False, na=False, regex=True)
            combined_mask = date_mask & keyword_mask
            filtered_data = data.loc[combined_mask]
    ...
    def plot_hashtag_frequencies_comparison(self, parquet_file: str, hashtags_list: List[str], start_date: str, end_date: str) -> None:
        """
        ...
        Args:
            parquet_file (str): Path to the Parquet file containing the data.
            hashtags_list (List[str]): A list of hashtags to compare (max 10).
            start_date (str): Start date in 'YYYY-MM-DD' format.
            end_date (str): End date in 'YYYY-MM-DD' format.
        """
        ...
        try:
            ...
            # Filter data by the overall date range
            date_mask_overall = (data['date'] >= start_date) & (data['date'] <= end_date)
            relevant_data = data[date_mask_overall]
    ...
    def plot_top_hashtags_trend(self, parquet_file: str, start_date: str, end_date: str, top_n: int = 10) -> None:
        """
        ...
        Args:
            ...
            start_date (str): Start date in 'YYYY-MM-DD' format.
            end_date (str): End date in 'YYYY-MM-DD' format.
            top_n (int): Number of top hashtags to identify and plot. Defaults to 10.
        """
        if top_n <= 0:
            print("Error: top_n must be a positive integer.")
            return
        ...
        try:
            ...
            # Determine the top N hashtags for the entire period
...
# --- Domain Specific Methods ---
    def top_domains_in_date_range(self, parquet_file_path: str, start_date: str, end_date: str, top_n: int = 20) -> Optional[pd.DataFrame]:
        """
        ...
        Parameters:
            ...
            start_date (str): Start date for filtering (inclusive).
            end_date (str): End date for filtering (inclusive).
            top_n (int): Number of top domains to return (default is 20).

        Returns:
            Optional[pd.DataFrame]: DataFrame containing top domains and their frequencies, or None if an error occurs.
        """
        try:
            # Read data from the Parquet file
            data = pd.read_parquet(parquet_file_path)
            ...
            # Group by 'domain' and sum frequencies
    ...
    def search_domain_in_date_range(self, parquet_file_path: str, domain_to_search: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        """
        ...
        Parameters:
            parquet_file_path (str): Path to the Parquet file containing the data.
            domain_to_search (str): The domain keyword to search for.
            start_date (str): Start date for filtering (inclusive).
            end_date (str): End date for filtering (inclusive).
        ...
        try:
            ...
            # Ensure 'domain' column exists and is string type
            if 'domain' not in data.columns or not pd.api.types.is_string_dtype(data['domain']):
            ...
            date_mask = (data['date'] >= start_date) & (data['date'] <= end_date)
            keyword_mask = data['domain'].str.contains(f'\\b{domain_to_search}\\b', case=False, na=False, regex=True)
            combined_mask = date_mask & keyword_mask
            filtered_data = data.loc[combined_mask]
    ...
    def search_domain_total_frequency_in_range(self, parquet_file_path: str, domain_keyword: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        """
        ...
        Parameters:
            parquet_file_path (str): Path to the Parquet file containing the data.
            domain_keyword (str): The domain keyword to search for.
            start_date (str): Start date for filtering (inclusive).
            end_date (str): End date for filtering (inclusive).
        ...
        try:
            ...
            # Apply date and domain filters
            date_mask = (data['date'] >= start_date) & (data['date'] <= end_date)
            keyword_mask = data['domain'].str.contains(f'\\b{domain_keyword}\\b', case=False, na=False, regex=True)
            combined_mask = date_mask & keyword_mask
            filtered_data = data.loc[combined_mask]
    ...
    def plot_domain_frequencies_comparison(self, parquet_file: str, domains_list: List[str], start_date: str, end_date: str) -> None:
        """
        ...
        Args:
            parquet_file (str): Path to the Parquet file containing the data.
            domains_list (List[str]): A list of domain keywords to compare (max 10).
            start_date (str): Start date in 'YYYY-MM-DD' format.
            end_date (str): End date in 'YYYY-MM-DD' format.
        """
        if not domains_list:
    ...
    def plot_top_domains_trend(self, parquet_file: str, start_date: str, end_date: str, top_n: int = 10) -> None:
        """
